...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement): {
      ...
      const _gl = gl as any;
      if (isWebGL2) {
        formatRGBA = getSupportedFormat(gl!, _gl.RGBA16F, gl!.RGBA, halfFloatTexType);
        formatRG = getSupportedFormat(gl!, _gl.RG16F, _gl.RG, halfFloatTexType);
        formatR = getSupportedFormat(gl!, _gl.R16F, _gl.RED, halfFloatTexType);
      } else {
    ...
    function updateColors(dt: number) {
      iridescentHue += dt * config.IRIDESCENT_SPEED;
      if (iridescentHue >= 1) iridescentHue -= 1;
    ...
    function iridescentColor(): [number, number, number] {
      const { r, g, b } = HSVtoRGB(iridescentHue, 1.0, 1.0);
